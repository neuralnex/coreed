  ...
  return (
    <>
      <nav className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-2rem)] max-w-5xl">
        ...
        {/* Mobile menu */}
        {showMobileMenu && (
          <div className="mt-2 md:hidden bg-[#0a0a0a]/90 backdrop-blur-xl border border-white/15 rounded-4xl shadow-lg shadow-black/60 px-6 py-4">
            <div className="flex flex-col gap-1">
              {NAV_LINKS.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="px-4 py-3 text-sm text-white/50 hover:text-white rounded-4xl hover:bg-white/5 transition-all"
              ...
              <div className="mt-2 pt-2 border-t border-white/5">
                {isConnected && address ? (
                  ...
                  <div className="flex flex-col gap-2 px-4 pt-2">
                    <Link
                      href="/docs"
                      className="w-full text-center px-4 py-3 text-sm font-medium text-white/60 border border-white/10 rounded-4xl"
                      onClick={() => setShowMobileMenu(false)}
                    >
                      Getting Started
                    </Link>
